...
enum ShutterModes { SHT_OFF_OPEN__OFF_CLOSE, SHT_OFF_ON__OPEN_CLOSE, SHT_PULSE_OPEN__PULSE_CLOSE, SHT_OFF_ON__OPEN_CLOSE_STEPPER, SHT_OFF_ON_PWM_SERVO};
...
void ShutterInit(void)
{
  ...
  for (uint32_t i = 0; i < MAX_SHUTTERS; i++) {
    ...
    } else if (pin[GPIO_PWM1+i] < 99) {
      Shutter.mode = SHT_OFF_ON_PWM_SERVO;
      analogWrite(pin[GPIO_PWM1+i], 50);
    } else {
...
void ShutterLimitRealAndTargetPositions(uint32_t i)
{
  if (Shutter.mode < SHT_OFF_ON__OPEN_CLOSE_STEPPER) {
    // for stepper/servo motors the COUNTER defines the position. To avoid shifting it is forbidden to change the
    // actual position
